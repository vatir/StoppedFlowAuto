...
def ForkDisplay(ADC_Intervals, Traces, Title = 'ADC Data Plot', XAxis='Time', YAxis = 'Intensity', *args, **kargs):
#    try:
#        __IPYTHON__
##         if ('__IPYTHON__' in globals()['__builtins__'].keys()):
#        return ADCPlot(ADC_Intervals, Traces, Title, XAxis, YAxis, kargs)
#    except:
#        pass
    from multiprocessing import Process
    proc = Process(target=ADCPlot, args=(ADC_Intervals, Traces, Title, XAxis, YAxis, kargs))
    proc.start()
    return proc
...
def SetupPlot(Title):
    ...
    #scale = 4.5
    scale = 5.0
    Fig = plt.figure(Title, (1.2*golden*scale, scale), dpi=dpi)
...
def ADCPlot(ADC_Intervals, Traces, Title = 'ADC Data Plot', XAxis='Time', YAxis = 'Intensity', *args):
    try:
        args = args[0]
    except:
        pass
...
#         use("GTK3Cairo", warn=False)
    ...
    if PlotCount == 1:
        axes.set_xlabel(r'\textbf{%s}'%str(XAxis))
